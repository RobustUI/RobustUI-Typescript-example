...
export abstract class RobustUISelectiveMachine implements RobustUI {
  ...
  protected machineSwitchStream = this.$machineSwitch
    .asObservable()
    .pipe(filter((e) => e != null));
  private machineElements: Map<string, HTMLElement> = new Map<
    string,
    HTMLElement
  >();
      ...
      el.outputs.forEach((output) => {
        el.getOutputStream(output.stream.toLowerCase()).subscribe((event) => {
          this.$outputStream.next({
            name: output.stream.toLowerCase(),
            value: event,
          });
        });
      });
  ...
  public onNewConfiguration(machine?: string): Observable<Configuration[]> {
    if (machine == null) {
      return this.configurations
        .asObservable()
        .pipe(filter((e) => e != null && e.length > 0));
    } else {
      return this.configurations
        .asObservable()
        .pipe(
  ...
  public sendInput(event: string, data: number): void {
    if (this.inputs.find((input) => input.stream === event) != null) {
      ...
      this.machines.get(this.activeMachine).sendInput(event, data);
    }
  }
